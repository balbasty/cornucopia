...
from .py import make_vector


def _inplace_softmax(Z, W=None, dim=0):

    Z = torch.movedim(Z, dim, 0)
...
def mrf_suffstat(Z, W=None, vx=1):
    ...
    dim = Z.dim() - 1
    K = len(Z)
    vx = make_vector(vx, dim, dtype=Z.dtype, device='cpu')
                ...
                for d in range(dim):
                    ...
                    for ko in range(K):
                        for ki in range(K):
                            ...
                            if W is not None:
                                logP1 = logP1.item() * ivx1
                                Z0[(*slicer_pre, ko)].addcmul_(
                                    Wpre, pre[..., ki], value=logP1
                                )
                                Z0[(*slicer_post, ko)].addcmul_(
                                    Wpost, post[..., ki], value=logP1
                                )
                            else:
                                logP1 = logP1 * ivx1
                                Z0[(*slicer_pre, ko)].addcmul_(
...
def mrf_conv(Z, logP, L=None, W=None, vx=1, max_iter=5, tol=1e-4):
    """Apply a Markov Random Field via convolution (i.e. Gauss-Seidel-like)
    ...
    # If MRF log-prior has an implicit class, make it explicit
    if len(logP) == K-1:
        logP0 = logP
        logP = logP0.new_zeros([K, K])
        logP[1:] = logP0
    ...
    kernel = torch.zeros([K, K] + [3]*dim, dtype=Z.dtype, device=Z.device)
    kernel[:, :] = logP[(Ellipsis,) + (None,)*dim]
    kernel[(Ellipsis, ) + (1,)*dim] = 0

    # Weight by inverse voxel size
    vx = make_vector(vx, dim, dtype=Z.dtype, device='cpu')
    ivx = vx.reciprocal()
    ivxw = ivx.new_ones([])
    for d in range(dim):
        ivxw = ivxw[..., None] * ivx[d].expand([3])
    kernel *= ivxw
    ...
    for n_iter in range(max_iter):
        ...
        # Compute log-prior term via convolution
        Z = convnd(dim, Z, kernel, padding='same', bound='replicate')
...
def mrf_sample(logP, shape=None, M=None, vx=1, nb_iter=100, implicit=False):
    ...
    if M is None:
        ...
        M0[1:] = M
        L = Cat(logits=torch.movedim(M0, 0, -1)).sample()
        del M0
    else:
        L = Cat(logits=torch.movedim(M, 0, -1)).sample()

    vx = make_vector(vx, dim, dtype=logP.dtype, device='cpu')
    ivx = vx.reciprocal().tolist()
                ...
                for d in range(dim):
                    ...
                    for ko in range(K):
                        for ki in range(K):
                            ...
                            P0[(ko, *slicer_pre)].add_(
                                pre == ki, alpha=logP1
                            )
                            P0[(ko, *slicer_post)].add_(
                                post == ki, alpha=logP1
                            )

            sampler = Cat(logits=torch.movedim(P0, 0, -1))
